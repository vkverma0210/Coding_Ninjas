...
void build(int *tree, int *arr, int s, int e, int i, int *pow2mod3)
{
    ...
    build(tree, arr, s, mid, 2 * i, pow2mod3);
    build(tree, arr, mid + 1, e, 2 * i + 1, pow2mod3);

    tree[i] = (tree[2 * i] * pow2mod3[e - mid] + tree[2 * i + 1]) % 3;
}
...
int query_zero(int *tree, int s, int e, int i, int l, int r, int *pow2mod3)
{
    ...
    int left = query_zero(tree, s, mid, 2 * i, l, r, pow2mod3);
    int right = query_zero(tree, mid + 1, e, 2 * i + 1, l, r, pow2mod3);
...
void query_one(int *tree, int *arr, int s, int e, int i, int *pow2mod3, int index)
{
    ...
    if (index > mid)
    {
        query_one(tree, arr, mid + 1, e, 2 * i + 1, pow2mod3, index);
    }
    else
    {
        query_one(tree, arr, s, mid, 2 * i, pow2mod3, index);
    }

    tree[i] = (tree[2 * i + 1] + tree[2 * i] * pow2mod3[e - mid]) % 3;
}
